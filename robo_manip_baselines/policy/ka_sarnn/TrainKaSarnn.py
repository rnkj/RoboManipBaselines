...
class TrainKaSarnn(TrainBase):
    DatasetClass = KaSarnnDataset
    ...
    def set_additional_args(self, parser):
        ...
        #     nargs="+",
        #     default=[],
        #     help="List of expert knowledge names (worker_action_classes | robot_action_classes)",
        # )
        ...
        parser.add_argument(
            ...
            type=int,
            default=50,
            help="Dimension of hidden state of knowledge adapter",
        )
    ...
    def setup_model_meta_info(self):
        ...
        with RmbData(rmb_file, mode="r") as rmb_data:
            num_expert_knowledges = [
                len(rmb_data.attrs[DataKey.WORKER_ACTIONS]),
                len(rmb_data.attrs[DataKey.ROBOT_ACTIONS]),
            ]
        self.model_meta_info["expert_knowledge"] = {
            "keys": [DataKey.WORKER_ACTION_CLASSES, DataKey.ROBOT_ACTION_CLASSES],
            "num_classes": num_expert_knowledges,
        }
    ...
    def setup_policy(self):
        ...
        self.model_meta_info["policy"]["args"] = {
            ...
            "adapter_hidden_dim": self.args.adapter_hidden_dim,
        }

        # Construct policy
        self.policy = KaSarnnPolicy(
    ...
    def calc_loss(
        ...
        num_agents = len(action_indices_list)
        lstm_state = None
        adapter_state = None
        predicted_state_seq = []
        ...
        logits_seq_list = [[] for _ in range(num_agents)]
        for time_idx in range(len(state_seq[0]) - 1):
            ...
            for agent_idx in range(num_agents):
                logits_seq_list[agent_idx].append(logits[agent_idx])
        ...
        for agent_idx in range(num_agents):
            logits_seq_list[agent_idx] = torch.permute(
                torch.stack(logits_seq_list[agent_idx]),
                (1, 0, 2),
            )  # (batch_size, episode_len, num_classes)
        ...
        for agent_idx in range(num_agents):
            target = F.one_hot(
                action_indices_list[agent_idx][:, :-1],
                num_classes=self.model_meta_info["expert_knowledge"]["num_classes"][
                    agent_idx
                ],
            ).float()
            target = (
                target * (1 - self.args.label_smoothing)
                + (1 - target) * self.args.label_smoothing
            )
            logits_loss = torch.mean(bce(logits_seq_list[agent_idx], target), dim=-1)
            logits_loss = torch.sum(logits_loss * mask_seq[:, :-1]) / torch.sum(
                mask_seq[:, :-1]
            )
            logits_loss_list.append(logits_loss)
        ...
        loss = (
            ...
            * torch.sum(torch.stack(attention_loss_list))
            + self.args.logits_loss_scale * torch.sum(torch.stack(logits_loss_list))
        )
